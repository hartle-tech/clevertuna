...
final class AppDelegate: NSObject, NSApplicationDelegate, AppCommands {
    /// Held so it is not deallocated the moment launch finishes.
    ...
    func applicationDidFinishLaunching(_ note: Notification) {
        ...
        // The rotation's heartbeat. A minute is finer than any rule the engine
        // offers — the shortest is five — so a change lands within a minute of
        // being due, and the other fifty-nine ticks cost a subprocess that
        // decides nothing is due and exits.
        rotation = Timer.scheduledTimer(withTimeInterval: 60, repeats: true) { _ in
            Task { @MainActor in await AppState.shared.model.rotateTick() }
        }
        ...
        Task { @MainActor in
            ...
            await AppState.shared.model.rotateTick()
