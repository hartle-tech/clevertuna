...
final class BuilderModel {
    ...
    /// The function row, and what each key can be set to.
    var keyMap: KeyMap?
...
extension BuilderModel {
    ...
    /// Let the clock change the lighting, if a rotation is set.
    ///
    /// The engine lives in the core and is pure — a plan and a timestamp go in,
    /// a theme or nothing comes out — and it opens no connection unless the
    /// slot has actually changed. What it never had on macOS was anything to
    /// ask it: the module was written expecting the menu bar app to tick it,
    /// and the menu bar app never did, so `rotate every` worked from a terminal
    /// and did nothing for anybody using the app. This is that tick.
    ///
    /// Skipped while something else is talking to the keyboard: the hardware
    /// grants one connection, and a rotation is never worth interrupting a
    /// write a person is waiting on.
    func rotateTick() async {
        guard busy == nil else { return }
        guard (try? await device.rotateTick()) != nil else { return }
    }
...
extension BuilderModel {
    ...
    ///
    /// Read separately from the rest of the settings because it is a different
    /// shape — a list of keys rather than a list of switches — and because a
    /// keyboard without a remappable row should show the other panes rather
    /// than an error.
    func loadKeyMap() async {
        keyMap = try? await device.keyMap()
    }

    func setKey(_ key: String, to action: String, named: String) async {
        let done = await working("Remapping \(key.uppercased())", from: key) {
            try await device.setKey(key, to: action)
        }
        guard done != nil else { return }
        await loadKeyMap()
        status = "\(key.uppercased()) is now \(named)"
    }
