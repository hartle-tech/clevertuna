...
protocol DeviceService: Sendable {
    func rotateTick() async throws
    /// What each function key sends, and the actions one can be set to.
    func keyMap() async throws -> KeyMap
    func setKey(_ key: String, to action: String) async throws
...
actor RustCoreBackend: DeviceService {
    ...
    /// Apply the rotation if it is due.
    ///
    /// Cheap when it is not: the engine is `plan + a timestamp → a theme, or
    /// nothing`, and it opens no connection unless the slot has actually
    /// changed. Which is why this can be asked once a minute forever.
    func rotateTick() async throws {
        _ = try await run(["rotate", "tick"])
    }

    func keyMap() async throws -> KeyMap {
        try JSONDecoder().decode(KeyMap.self, from: try await run(["--json", "keys"]))
    }

    func setKey(_ key: String, to action: String) async throws {
        _ = try await run(["keys", key, action])
    }
...
/// The function row, as the core describes it.
///
/// The names and the list of what a key can do are the core's — this is the
/// same rule the builder follows for effects. A window that kept its own table
/// of HID usages would be a second answer to a question `keymap.rs` already
/// answers, and the two would drift.
struct KeyMap: Decodable, Sendable {
    struct Row: Decodable, Sendable, Identifiable, Hashable {
        /// `f1` … `f12`.
        let key: String
        /// `F1` … `F12`.
        let name: String
        /// What it sends now, in words.
        let label: String
        /// The action id, or nil when it is something with no name here.
        let action: String?
        var id: String { key }
    }
    struct Choice: Decodable, Sendable, Hashable {
        let id: String
        let name: String
    }
    let keys: [Row]
