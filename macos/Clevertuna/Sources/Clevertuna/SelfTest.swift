...
enum SelfTest {
    ...
    static func run() async -> Never {
        ...
        await check("the keyboard layout loads and fits together") {
            ...
            // 79, counted off the photographs and corrected against the
            // hardware: six rows, no navigation column, the arrow cluster's up
            // and down sharing one column, tab carrying the AI key, and caps
            // lock carrying its indicator rather than a key beside it. A number
            ...
            guard keys == 79 else { return "expected 79 keys, found \(keys)" }
