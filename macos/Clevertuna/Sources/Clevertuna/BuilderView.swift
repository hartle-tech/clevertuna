    ...
    private var inspector: some View {
        ...
        return VStack(alignment: .leading, spacing: DS.S.groupGap) {
            ...
            if let zone = model.zone {
                ...
                // The aurora is the firmware's own animation and carries no
                // colour field, so the deck is drawing an impression of it
                // rather than a reading. Saying so beats letting somebody
                // compare it against the keys and conclude the app is broken.
                if zone.effect == "aurora" {
                    Text("The keyboard draws this one itself. The preview is an "
                         + "impression — its colours are not read from the device.")
                        .font(DS.F.secondary)
                ...
                // Only if it will hold something. The aurora animates but takes
                // no speed, no stretch and no direction, so keying this off
                // `animated` drew an empty box with a heading over it.
                if offer?.speed == true || offer?.length == true || offer?.gradient == true {
