...
struct DeckView: View {
    ...
    /// What the deck draws *outside* its own rectangle.
    ///
    /// The tray stands 7pt proud on every side and its selection outline is
    /// another 2.5 on top of that. None of it is part of `deckRect`, so a deck
    /// sized to fill the room puts its own border under the window's edge —
    /// which is what had the keyboard touching the left side of the builder.
    private var bezel: CGFloat { 13 }

    /// The deck keeps the keyboard's own proportions and is centred in whatever
    /// room it is given, less the bezel it draws around itself.
    private func deckRect(in size: CGSize) -> CGRect {
        let room = CGSize(width: max(size.width - bezel * 2, 1),
                          height: max(size.height - bezel * 2, 1))
        var w = room.width
        ...
        // Centred in the full size, so the bezel falls evenly on both sides.
    ...
    private struct Cap: Identifiable {
        ...
        let point: Double
        let homing: Bool
        /// Caps lock has a status light of its own, on the keycap. It is not a
        /// key — it was drawn as one until the hardware was looked at.
        let led: Bool
    ...
    private func cap(_ c: Cap, in g: GraphicsContext) {
        ...
        // Caps lock's own status light, on the right of the cap.
        if c.led {
            let d = max(2.5, min(4.5, c.rect.height * 0.11))
            g.fill(Path(ellipseIn: CGRect(x: c.rect.maxX - d * 2.4,
                                          y: c.rect.midY - d / 2,
                                          width: d, height: d)),
                   with: .color(finish.legend.opacity(0.55)))
        }
    }
