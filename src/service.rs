...
/// What the AI key is doing.
///
/// `Request.Type.GET_AI_STATE` is 7, and the request submessage that carries it
/// is field 9 — every other type in this file sits at `type + 2` (8→10, 9→11,
/// 10→12, 12→14), which is four for four and the only pattern the numbering
/// offers. Read-only, so a wrong guess costs a `BAD_REQUEST` and nothing else.
///
/// Returns the response's own fields, undecoded: nothing here knows what an AI
/// state looks like yet, and inventing a shape for it would be the same mistake
/// as inventing the aurora's colours.
pub fn get_ai_state(dev: &mut Device) -> Result<Vec<(u32, String)>> {
    const GET_AI_STATE: u64 = 7;
    const REQ_GET_AI_STATE: u32 = 9;
    let mut req = proto::varint_field(REQ_TYPE, GET_AI_STATE);
    req.extend(proto::bytes_field(REQ_GET_AI_STATE, &[]));
    let raw = dev.request(&req)?;
    let resp = proto::parse(&raw).ok_or_else(|| Error::Protocol("unparsable response".into()))?;
    check_bad_request(&resp)?;
    let mut out = Vec::new();
    for (field, values) in &resp {
        for v in values {
            out.push((
                *field,
                match v {
                    proto::Value::Varint(n) => format!("varint {}", n),
                    proto::Value::Bytes(b) => format!("{} bytes: {}", b.len(), hexdump(b)),
                    other => format!("{:?}", other),
                },
            ));
        }
...
fn hexdump(b: &[u8]) -> String {
    b.iter().take(48).map(|x| format!("{:02x}", x)).collect::<Vec<_>>().join("")
}
