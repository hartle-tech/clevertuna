...
impl Look {
    ...
    pub fn palette(&self) -> Vec<[u8; 3]> {
        if self.effect == Effect::Aurora {
            // The aurora's colours are the firmware's, not ours: it carries no
            // colour field, so nothing here is ever written to the device.
            //
            // This list was invented to give a picker something to draw, and
            // then the deck started painting the keys with it — so a guess
            // became a claim about what the hardware does. Observed on a CLVX
            // S: the keys never go purple under the aurora. The two violet
            // entries that used to be here ([80,90,255] and [200,60,255]) are
            // therefore wrong and are gone; what is left is the part of the
            // guess the hardware does not contradict.
            //
            // ⚠ Still an impression, not a reading. The real sequence has not
            // been measured, and until it is, no preview of this effect should
            // be trusted for colour.
            return vec![[0, 200, 255], [40, 220, 180], [0, 200, 255], [90, 210, 220]];
        }
